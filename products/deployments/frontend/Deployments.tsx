...
import { LemonButton, LemonDialog, LemonTable, LemonTableColumns, LemonTag, PaginationManual } from '@posthog/lemon-ui'
...
export function Deployments(): JSX.Element {
    const { deployments, allDeploymentsLoading, currentDeployment, filters, shouldShowEmptyState } =
        useValues(deploymentsLogic)
    const { setFilters, redeployDeployment, rollbackDeployment } = useActions(deploymentsLogic)
                ...
                <More
                    overlay={
                        <>
                            ...
                            {d.deployment_url && (
                                <LemonButton fullWidth to={d.deployment_url} targetBlank>
                                    View live
                                </LemonButton>
                            )}
                        </>
    ...
    return (
        <SceneContent>
            ...
            <ProductIntroduction
                ...
                description="Track and manage every deployment of your app — see status, duration, who shipped it, and instantly redeploy or roll back."
                isEmpty={shouldShowEmptyState}
                action={() =>
                    LemonDialog.open({
                        title: 'Connect a repo',
                        description:
                            'Deployment ingestion is coming soon. For now this is a preview powered by sample data.',
            ...
            {!shouldShowEmptyState && (
                <>
                    <DeploymentsFilters />
                    <LemonTable
                        dataSource={deployments.results}
                        columns={columns}
                        loading={allDeploymentsLoading}
